...
public class AVLTree extends SearchTree {         // An AVLTree is a SearchTree
	...
	private Node rotateLeft(Node oldCurrentRoot) { // The rotation to the left
		Node newCurrentRoot = (Node) oldCurrentRoot.right; // Determine the new current root
	...
	private Node rotateRight(Node oldCurrentRoot) { // The rotation to the right
		Node newCurrentRoot = (Node) oldCurrentRoot.left; // Determine the new current root
	...
	private Node balanceLeftLoaded(Node currentRoot) { // Balance a left-loaded
		...
		if (currentRoot.balance == -2) {                   // Is there anything to do?
			switch(((Node) currentRoot.left).balance) {    // Is the AVL-requirement
			case -1:                                       // injured?
			...
			case +1:
				rotateLeft((Node) currentRoot.left);
	...
	private Node balanceRightLoaded(Node currentRoot) { // Balance a right-loaded
		...
		if (currentRoot.balance == 2) {                   // Is there anything to do?
			switch(((Node) currentRoot.right).balance) {  // Is the AVL-requirement
			case +1:                                      // injured?
			...
			case 0:                                       // one of the three possible
				...
				break;                                    // according to the theory
			case -1:
				rotateRight((Node) currentRoot.right);
	...
	protected void grownTo(Node node) { // Message to all parents that a sub
		Node parent = (Node) node.parent;     // tree indicated by the passed node has
	...
	public void insert(Object data) {                 // The overwritten insert-
		Node newAVLNode = new Node(null, data, null); // method reports the growth
	...
	protected void remove(Node toRemove) { // The overwritten remove-method reports the
		if (toRemove != null) {            // contraction of a sub-tree up to the parents
			toRemove = (Node) replaceRoot(toRemove);
			shrunkBy((Node) toRemove);
			removeLeaf(toRemove);
		}
	}
